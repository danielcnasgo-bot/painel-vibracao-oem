...
limites_alarme_ug1 = {
    '50-100MW':  {'OMGG-0':127.4375,'OMGG-90':154.71, 'OMGG-A':360.83, 'VMGG-0':0.60,'VMGG-90':0.63,'VMCG-A':0.85, 'OMGT-0':134.64,'OMGT-90':137.50,'VMGT-0':1.38,'VMGT-90':1.21,'VCGR':2.00},
    '101-150MW': {'OMGG-0':122.70, 'OMGG-90':149.38, 'OMGG-A':373.62, 'VMGG-0':0.75,'VMGG-90':0.75,'VMCG-A':1.22, 'OMGT-0':137.50,'OMGT-90':137.50,'VMGT-0':1.38,'VMGT-90':1.38,'VCGR':2.11},
    '151-200MW': {'OMGG-0':137.43, 'OMGG-90':164.89, 'OMGG-A':375.55, 'VMGG-0':0.75,'VMGG-90':0.75,'VMCG-A':1.73, 'OMGT-0':137.50,'OMGT-90':137.50,'VMGT-0':1.38,'VMGT-90':1.38,'VCGR':2.02},
}
limites_trip_ug1 = {
    '50-100MW':  {'OMGG-0':224,'OMGG-90':224,'OMGG-A':451.035,'VMGG-0':0.75,'VMGG-90':0.78,'VMCG-A':1.065,'OMGT-0':252,'OMGT-90':252,'VMGT-0':1.65,'VMGT-90':1.52,'VCGR':2.51},
    '101-150MW': {'OMGG-0':224,'OMGG-90':224,'OMGG-A':467.025,'VMGG-0':0.84,'VMGG-90':0.79,'VMCG-A':1.53, 'OMGT-0':252,'OMGT-90':252,'VMGT-0':1.73,'VMGT-90':1.72,'VCGR':2.64},
    '151-200MW': {'OMGG-0':224,'OMGG-90':224,'OMGG-A':469.44, 'VMGG-0':1.10,'VMGG-90':0.88,'VMCG-A':2.16, 'OMGT-0':252,'OMGT-90':252,'VMGT-0':1.54,'VMGT-90':1.59,'VCGR':2.52},
}
limites_alarme_ug2 = {
    '50-100MW':  {'OMGG-0':212.5, 'OMGG-90':212.5, 'OMGG-A':301.88,'VMGG-0':0.54,'VMGG-90':0.43,'VMCG-A':0.90,'OMGT-0':84.04, 'OMGT-90':77.58, 'VMGT-0':1.38,'VMGT-90':1.38,'VCGR':2.70},
    '101-150MW': {'OMGG-0':212.50,'OMGG-90':212.50,'OMGG-A':296.48,'VMGG-0':0.65,'VMGG-90':0.56,'VMCG-A':0.85,'OMGT-0':102.10,'OMGT-90':104.43,'VMGT-0':1.38,'VMGT-90':1.38,'VCGR':3.14},
    '151-200MW': {'OMGG-0':212.5, 'OMGG-90':212.5, 'OMGG-A':292.04,'VMGG-0':0.75,'VMGG-90':0.60,'VMCG-A':1.10,'OMGT-0':124.59,'OMGT-90':125.53,'VMGT-0':1.38,'VMGT-90':1.38,'VCGR':4.80},
}
limites_trip_ug2 = {
    '50-100MW':  {'OMGG-0':252,'OMGG-90':252,'OMGG-A':377.355,'VMGG-0':0.67,'VMGG-90':0.53,'VMCG-A':1.125,'OMGT-0':252,'OMGT-90':252,'VMGT-0':2.05,'VMGT-90':1.71,'VCGR':3.38},
    '101-150MW': {'OMGG-0':252,'OMGG-90':252,'OMGG-A':370.605,'VMGG-0':0.81,'VMGG-90':0.70,'VMCG-A':1.07, 'OMGT-0':252,'OMGT-90':252,'VMGT-0':2.18,'VMGT-90':1.86,'VCGR':3.93},
    '151-200MW': {'OMGG-0':252,'OMGG-90':252,'OMGG-A':365.055,'VMGG-0':0.78,'VMGG-90':0.75,'VMCG-A':1.38, 'OMGT-0':252,'OMGT-90':252,'VMGT-0':1.83,'VMGT-90':1.88,'VCGR':6.00},
}
...
if uploaded_file is not None:
    with st.spinner("Processando dados, gerando dashboard HTML e exportando Imagens HD. Isso levará alguns segundos..."):
        ...
        col_data = 'Data'
        col_pot  = f'CF.{pref}.GER.SYN.ACTIVE_POWER_P1'

        # --- Limpeza e conversão de tipos ---
        for df in [df_presente, df_passado]:
            if col_pot in df.columns:
                df[col_pot] = pd.to_numeric(
                    df[col_pot].astype(str).str.replace(',', '.'), errors='coerce'
                )
        ...
        # --- FIX: máscara de potência zero aplicada em AMBOS os dataframes,
        #          usando NaN (não 0.0) para excluir do cálculo estatístico ---
        for df in [df_presente, df_passado]:
            if col_pot in df.columns:
                mask_pot_zero = (df[col_pot].fillna(0) == 0)
                for col_vib in colunas_vibracao:
                    if col_vib in df.columns:
                        df.loc[mask_pot_zero, col_vib] = np.nan  # NaN exclui da estatística
        ...
        def calcular_linha_historica(pot_atual):
            """Calcula a média dos 90% inferiores do histórico para a janela de potência dada."""
            resultado_vazio = pd.Series({c + '_media_p90': np.nan for c in todas_as_colunas})

            # FIX: ignora potência zero ou inválida
            if pd.isna(pot_atual) or pot_atual <= 0:
                return resultado_vazio

            pot_inteira = int(pot_atual)
            if pot_inteira in _cache_historico:
                return _cache_historico[pot_inteira]

            # FIX: filtra fora registros com potência zero/nula do histórico
            df_passado_valido = df_passado[df_passado[col_pot].fillna(0) > 0]

            filtro = (
                (df_passado_valido[col_pot] >= pot_inteira - JANELA_MW) &
                (df_passado_valido[col_pot] <= pot_inteira + JANELA_MW)
            )
            df_janela = df_passado_valido.loc[filtro]
            ...
            for c in todas_as_colunas:
                if c in df_janela.columns:
                    ...
                    if c in colunas_temperatura:
                        s = s[s <= 100.0]
                    resultados[c + '_media_p90'] = calcular_media_p90(s)
                else:
        ...
        # ==========================================
        # CONSTRUÇÃO DO HTML DO DASHBOARD
        # ==========================================
        html_content = f"""<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>Dashboard - {UNIDADE_ALVO}</title>
...
"""
        ...
        for i, macro_nome in enumerate(estrutura_abas.keys()):
            ...
            html_content += f'<button class="macro-links {active}" onclick="openMacro(event, \'{macro_nome}\')">{macro_nome}</button>\n'
        html_content += '</div>\n'
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            for i, (macro_nome, micros) in enumerate(estrutura_abas.items()):
                ...
                for j, micro_nome in enumerate(micros.keys()):
                    ...
                    id_micro_btn = f"{macro_nome}_{micro_nome.replace('.','')}"
                    html_content += (
                        f'<button class="micro-links {macro_nome}-links {active}" '
                        f'onclick="openMicro(event, \'{id_micro_btn}\', \'{macro_nome}\')">'
                        f'{micro_nome}</button>\n'
                    )
                html_content += '</div>\n'
                ...
                for j, (micro_nome, cols) in enumerate(micros.items()):
                    ...
                    # Prepara dataframes para boxplot
                    ...
                    # FIX: remove linhas com potência zero/nula antes do boxplot
                    df_box = df_box[df_box[col_pot].fillna(0) > 0]

                    df_box['Range'] = pd.cut(df_box[col_pot], bins=bins_potencia, labels=labels_potencia_base)

                    df_box_melt = df_box.melt(
                        id_vars=[col_data, col_pot, 'Periodo', 'Range'],
                        value_vars=cols_existentes,
                        var_name='ColunaOriginal',
                        value_name='Valor'
                    ).dropna(subset=['Range', 'Valor'])
                    ...
                    df_box_melt['n_amostras'] = df_box_melt.groupby(
                        ['Periodo', 'Range', 'Eixo'], observed=False
                    )['Valor'].transform('count')
                    df_box_melt = df_box_melt[df_box_melt['n_amostras'] >= 20]

                    df_atual_melt = df_box_melt[df_box_melt['Periodo'] == 'Semana Atual'].copy()
                    ...
                    # -------------------------------------------------------
                    # GRÁFICO 1: Boxplot exploratório (Histórico vs Semana Atual)
                    # -------------------------------------------------------
                    fig_box = px.box(
                        df_box_melt, x='Range', y='Valor', color='Periodo', facet_col='Eixo',
                        points='outliers',
                    ...
                    if not is_temp:
                        for col_idx, col_name in enumerate(cols_existentes, start=1):
                            ...
                            alarms = [limites_alarme[r].get(sn) for r in labels_potencia_base]
                            trips  = [limites_trip[r].get(sn)   for r in labels_potencia_base]
                            if any(a is not None for a in alarms):
                                fig_box.add_trace(go.Scatter(
                                    x=labels_potencia_base, y=alarms, mode='markers',
                                    marker=dict(symbol='line-ew', size=40, color='#FFCC00', line=dict(color='#FFCC00', width=4)),
                                    showlegend=False,
                                ), row=1, col=col_idx)
                            if any(t is not None for t in trips):
                                fig_box.add_trace(go.Scatter(
                                    x=labels_potencia_base, y=trips, mode='markers',
                                    marker=dict(symbol='line-ew', size=40, color='red', line=dict(color='red', width=4)),
                                    showlegend=False,
                                ), row=1, col=col_idx)

                    id_box = f"plot_{uuid.uuid4().hex}"
                    html_content += f'<div class="grafico-container">{fig_box.to_html(full_html=False, include_plotlyjs=False, div_id=id_box)}</div>\n'

                    # -------------------------------------------------------
                    # GRÁFICO 2: Boxplot executivo com P99
                    # -------------------------------------------------------
                    fig_exec = px.box(
                        df_atual_melt, x='Range_Plotly', y='Valor', facet_col='Eixo',
                        points='outliers',
                        title=f'Visão Geral - {macro_nome} {micro_nome}',
                        category_orders={"Range_Plotly": list(labels_base_com_n.values()), "Eixo": eixos_unicos},
                        color_discrete_sequence=['#1f77b4'],
                        hover_data=[col_pot],
                        labels={'Valor': f'Amplitude ({unidade_medida})'},
                    )
                    ...
                    for col_idx, col_name in enumerate(cols_existentes, start=1):
                        ...
                        for r_base, r_plotly in zip(labels_potencia_base, labels_base_com_n.values()):
                            df_filtered = df_atual_melt[
                                (df_atual_melt['Range'] == r_base) &
                                (df_atual_melt['ShortName'] == sn)
                            ]
                            if not df_filtered.empty:
                                p99_val = calcular_p99_com_trava(df_filtered['Valor'])
                                if pd.notna(p99_val):
                                    pos = labels_potencia_base.index(r_base)
                                    fig_exec.add_shape(
                                        type="line",
                                        x0=pos - 0.4, x1=pos + 0.4,
                                        y0=p99_val,   y1=p99_val,
                                        line=dict(color="purple", width=2, dash="dash"),
                                        row=1, col=col_idx,
                                    )
                                    fig_exec.add_annotation(
                                        x=r_plotly, y=p99_val,
                                        text=f"<b>P99={p99_val:.2f}</b>",
                        ...
                        if not is_temp:
                            ...
                            if any(a is not None for a in alarms):
                                fig_exec.add_trace(go.Scatter(
                                    x=list(labels_base_com_n.values()), y=alarms, mode='markers',
                                    marker=dict(symbol='line-ew', size=40, color='#FFCC00', line=dict(color='#FFCC00', width=4)),
                                    showlegend=False,
                                ), row=1, col=col_idx)
                            if any(t is not None for t in trips):
                                fig_exec.add_trace(go.Scatter(
                                    x=list(labels_base_com_n.values()), y=trips, mode='markers',
                                    marker=dict(symbol='line-ew', size=40, color='red', line=dict(color='red', width=4)),
                                    showlegend=False,
                                ), row=1, col=col_idx)

                    id_exec = f"plot_{uuid.uuid4().hex}"
                    html_content += f'<div class="grafico-container">{fig_exec.to_html(full_html=False, include_plotlyjs=False, div_id=id_exec)}</div>\n'

                    # -------------------------------------------------------
                    # IMAGEM ESTÁTICA: Boxplot executivo (PNG para ZIP)
                    # -------------------------------------------------------
                    fig_static_box, axes_box = plt.subplots(1, len(eixos_unicos), figsize=(24, 12), sharey=True)
                    if len(eixos_unicos) == 1:
                        axes_box = [axes_box]
                    ...
                    for ax_idx, eixo_nome in enumerate(eixos_unicos):
                        ...
                        if not df_eixo.empty:
                            sns.boxplot(
                                data=df_eixo, x='Range_Sns', y='Valor',
                                order=list(labels_sns_atual.values()),
                                ax=ax, color='#8AB1D4', width=0.5, fliersize=0, whis=(0, 100),
                            )
                            sns.stripplot(
                                data=df_eixo, x='Range_Sns', y='Valor',
                                order=list(labels_sns_atual.values()),
                                ax=ax, color='#1f77b4', alpha=0.6, jitter=True, size=4,
                            )
                        ...
                        for pos_x, (r_base, r_sns) in enumerate(zip(labels_potencia_base, labels_sns_atual.values())):
                            ...
                            if not df_filt.empty:
                                ...
                                if pd.notna(p99_val):
                                    ...
                                    y_max = df_eixo['Valor'].max()
                                    offset = y_max * 0.02 if pd.notna(y_max) and y_max != 0 else 0.01
                                    ax.text(pos_x, p99_val + offset, f'P99={p99_val:.2f}',
                                            color='purple', ha='center', va='bottom', fontweight='bold', fontsize=13)

                                if not is_temp:
                                    a = limites_alarme[r_base].get(sn)
                                    t = limites_trip[r_base].get(sn)
                                    if a:
                                        ax.hlines(a, pos_x - 0.3, pos_x + 0.3, colors='#FFCC00', lw=4,
                                                  label='Limite Alarme' if pos_x == 0 else "")
                                        total_alarme_viol += int((df_filt['Valor'] > a).sum())
                                    if t:
                                        ax.hlines(t, pos_x - 0.3, pos_x + 0.3, colors='red', lw=4,
                                                  label='Limite Trip' if pos_x == 0 else "")
                                        total_trip_viol += int((df_filt['Valor'] > t).sum())

                        if not is_temp:
                            from matplotlib.patches import Patch
                            handles, labels_leg = ax.get_legend_handles_labels()
                            handles.extend([
                                Patch(color='none', label=f'Excedeu Alarme: {total_alarme_viol}x'),
                                Patch(color='none', label=f'Excedeu Trip: {total_trip_viol}x'),
                            ])
                            ax.legend(handles=handles, loc='upper right', fontsize=14, frameon=True, facecolor='white')

                    plt.tight_layout()
                    img_buf_box = io.BytesIO()
                    fig_static_box.savefig(img_buf_box, format="png", dpi=250, bbox_inches='tight')
                    zip_file.writestr(f"01_Boxplot_Executivo_{macro_nome}_{micro_nome.replace('.','')}.png", img_buf_box.getvalue())
                    plt.close('all')
                    img_buf_box.close()
                    gc.collect()

                    # -------------------------------------------------------
                    # GRÁFICO 3: Tendência dinâmica (Plotly HTML)
                    # FIX: hovertemplate montado fora do f-string para evitar
                    #      escape incorreto das chaves do Plotly
                    # -------------------------------------------------------
                    if is_temp:
                        ...
                        cols_p90 = [c + '_media_p90' for c in cols_existentes if c + '_media_p90' in df_presente.columns]
                        media_historica_global = df_presente[cols_p90].mean(axis=1) if cols_p90 else pd.Series(np.nan, index=df_presente.index)

                        fig_tend = make_subplots(
                            rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.06,
                            subplot_titles=[
                                f"Temperatura (°C) - {nome_pior_sensor} (Atual vs Média Global Histórica)",
                                "Potência (MW)",
                            ],
                        )
                        # FIX: hovertemplate como string literal, sem f-string
                        hover_temp = "%{y:.2f} °C<br>Pot: %{customdata:.1f} MW<extra></extra>"
                        fig_tend.add_trace(go.Scatter(
                            x=df_presente[col_data],
                            y=df_presente[sensor_pior],
                            name=f"Atual: {nome_pior_sensor}",
                            line=dict(color='rgba(214, 39, 40, 0.8)', width=2),
                            customdata=df_presente[col_pot],
                            hovertemplate=hover_temp,
                        ), row=1, col=1)
                        fig_tend.add_trace(go.Scatter(
                            x=df_presente[col_data],
                            y=media_historica_global,
                            name="Histórico Global (90%)",
                            line=dict(color='green', dash='dot', width=2.5),
                        ), row=1, col=1)
                        fig_tend.update_yaxes(title_text="Temperatura (°C)", row=1, col=1)
                        fig_tend.add_trace(go.Scatter(
                            x=df_presente[col_data],
                            y=df_presente[col_pot],
                            name="Potência (MW)",
                            line=dict(color='orange', width=2),
                        ), row=2, col=1)
                        fig_tend.update_layout(
                            title=f'Tendência Dinâmica: {macro_nome} {micro_nome}',
                        ...
                        fig_tend = make_subplots(
                            rows=len(cols_existentes) + 1, cols=1,
                            shared_xaxes=True, vertical_spacing=0.06,
                            subplot_titles=titulos_sub,
                        )
                        for idx_col, col_val in enumerate(cols_existentes):
                            # FIX: hovertemplate como string literal
                            hover_vib = "%{y:.2f} " + unidade_medida + "<br>Pot: %{customdata:.1f} MW<extra></extra>"
                            fig_tend.add_trace(go.Scatter(
                                x=df_presente[col_data],
                                y=df_presente[col_val],
                                name="Atual",
                                line=dict(color='rgba(0, 0, 255, 0.4)', width=1.5),
                                customdata=df_presente[col_pot],
                                hovertemplate=hover_vib,
                            ), row=idx_col + 1, col=1)

                            col_hist = col_val + '_media_p90'
                            if col_hist in df_presente.columns:
                                fig_tend.add_trace(go.Scatter(
                                    x=df_presente[col_data],
                                    y=df_presente[col_hist],
                                    name="Média Hist. (90%)",
                                    line=dict(color='green', dash='dot', width=2.5),
                                ), row=idx_col + 1, col=1)
                        ...
                        fig_tend.add_trace(go.Scatter(
                            ...
                            name="Potência (MW)",
                            line=dict(color='orange', width=2),
                        ), row=len(cols_existentes) + 1, col=1)
                        fig_tend.update_layout(
                            title=f'Tendência Dinâmica: {macro_nome} {micro_nome}',
                            hovermode="x unified", template="plotly_white",
                            height=300 * (len(cols_existentes) + 1),
                            margin=dict(l=20, r=20, t=50, b=20),
                        )

                    id_tend = f"plot_{uuid.uuid4().hex}"
                    html_content += f'<div class="grafico-container">{fig_tend.to_html(full_html=False, include_plotlyjs=False, div_id=id_tend)}</div>\n'

                    # -------------------------------------------------------
                    # IMAGEM ESTÁTICA: Tendência (PNG para ZIP)
                    # -------------------------------------------------------
                    if is_temp:
                        ...
                        ax_temp.plot(df_presente[col_data], df_presente[sensor_pior],
                                     color='red', linewidth=2, label=f"Atual: {nome_pior_sensor} (Pior)")
                        ax_temp.plot(df_presente[col_data], media_historica_global,
                                     color='green', linestyle='--', linewidth=2.5, label="Histórico Global (90%)")
                        ...
                        for ax_idx, col_val in enumerate(cols_existentes):
                            ax = axes_tend[ax_idx]
                            ax.plot(df_presente[col_data], df_presente[col_val],
                                    color='blue', alpha=0.5, linewidth=1.5, label='Valor Atual')
                            col_hist = col_val + '_media_p90'
                            if col_hist in df_presente.columns:
                                ax.plot(df_presente[col_data], df_presente[col_hist],
                                        color='green', linestyle='--', linewidth=2.5, label='Média Hist. (90%)')
                    ...
                    img_buf_tend = io.BytesIO()
                    fig_static_tend.savefig(img_buf_tend, format="png", dpi=250, bbox_inches='tight')
                    zip_file.writestr(f"02_Tendencia_Executiva_{macro_nome}_{micro_nome.replace('.','')}.png", img_buf_tend.getvalue())
                    plt.close('all')
                    img_buf_tend.close()
                    gc.collect()

                    # -------------------------------------------------------
                    # TABELA DE AUDITORIA DA MÉDIA DE 90%
                    # -------------------------------------------------------
                    tabela_auditoria = []
                    df_passado_valido_aud = df_passado[df_passado[col_pot].fillna(0) > 0]
                    for p_alvo in range(50, 205, 5):
                        df_janela = df_passado_valido_aud.loc[
                            (df_passado_valido_aud[col_pot] >= p_alvo - JANELA_MW) &
                            (df_passado_valido_aud[col_pot] <= p_alvo + JANELA_MW)
                        ]
                        if len(df_janela) > 0:
                            linha = {
                                'Potência Específica (MW)': f"{p_alvo} MW",
                                'Janela de Cálculo (MW)':   f"{p_alvo - 5} a {p_alvo + 5}",
                                'Amostras Úteis (n)':        len(df_janela),
                            }
                    ...
                    html_content += (
                        f'<div class="grafico-container" style="padding-bottom: 50px;">'
                        f'<h3 style="text-align: center; color: #555;">'
                        f'Tabela de Auditoria do Histórico: Média de 90% dos Dados ({macro_nome} {micro_nome})'
                        f'</h3>'
                        f'{df_tabela_aud.to_html(index=False, classes="styled-table", justify="center")}'
                        f'</div>\n</div>\n'
                    )
            ...
            # -------------------------------------------------------
            # FECHAMENTO DO HTML COM JAVASCRIPT DE NAVEGAÇÃO
            # -------------------------------------------------------
            html_content += """
<script>
...
setTimeout(() => { window.dispatchEvent(new Event('resize')); }, 200);
            ...
            zip_file.writestr(
                f"00_Dashboard_Interativo_{UNIDADE_ALVO}.html",
