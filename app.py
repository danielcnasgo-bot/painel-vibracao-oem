...
# ==========================================
st.set_page_config(page_title="Painel de Confiabilidade O&M", layout="wide")
st.title("Painel de Confiabilidade O&M")
...
if uploaded_file is not None:
    with st.spinner("Processando dados e calculando baselines. Isso pode levar alguns segundos..."):
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            # --- CRIAÇÃO DAS ABAS MACRO NO STREAMLIT ---
            macro_tabs = st.tabs(list(estrutura_abas.keys()))
            
            for i, (macro_nome, micros) in enumerate(estrutura_abas.items()):
                with macro_tabs[i]:
                    
                    # --- CRIAÇÃO DAS ABAS MICRO NO STREAMLIT ---
                    micro_tabs = st.tabs(list(micros.keys()))
                    ...
                    for j, (micro_nome, cols) in enumerate(micros.items()):
                        with micro_tabs[j]:
                            ...
                            if not cols_existentes:
                                st.warning("Dados não encontrados para esta aba.")
                                continue
                            ...
                            # --- 2. HTML PLOTLY: BOXPLOT EXECUTIVO ---
                            ...
                            # Salva imagem diretamente no buffer do Zip
                            ...
                            # Salva imagem de tendência diretamente no buffer do Zip
                            ...
                            # --- 5. TABELA DE VALIDAÇÃO DA MÉDIA DE 90% ---
                            st.markdown(f"### Tabela de Auditoria do Histórico: Média de 90% ({macro_nome} {micro_nome})")
                            tabela_auditoria = []
                            ...
                            st.dataframe(pd.DataFrame(tabela_auditoria), use_container_width=True)

        st.success("Análise concluída com sucesso!")
        
        # Botão para baixar as imagens para o relatório
        st.download_button(
            label="📦 Baixar Gráficos HD para o Relatório (.zip)",
            data=zip_buffer.getvalue(),
            file_name=f"Graficos_Relatorio_{UNIDADE_ALVO}.zip",
            mime="application/zip",
        )
else:
    st.info("Aguardando upload da planilha Excel para iniciar o dashboard.")
